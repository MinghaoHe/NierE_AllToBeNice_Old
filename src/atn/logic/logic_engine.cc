...
inline void CursorPosCallbackWrap(GLFWwindow *window, double xpos, double ypos) {
  UNUSED(window);
  std::invoke(&atn::logic::LogicEngine::CursorPosCallback, atn::base::GameEngine::Instance().GetLogic().get(),
              static_cast<float>(xpos), static_cast<float>(ypos));
}


inline void KeyCallbackWrap(GLFWwindow *window, int code_point, int scancode, int action, int mods) {
  UNUSED(window);
  UNUSED(scancode);
  UNUSED(mods);
  std::invoke(&atn::logic::LogicEngine::CharInputCallback, atn::base::GameEngine::Instance().GetLogic().get(),
...
LogicEngine::LogicEngine(base::WindowContext &window_context,
...
void LogicEngine::InitLogic() {
  ...
  // glfwSetInputMode(window_context_.window, GLFW_CURSOR, GLFW_CURSOR_DISABLED);
...
void LogicEngine::CursorPosCallback(float xpos, float ypos) {
  logic_context_.event.cursor_event.xpos =
          (xpos - static_cast<float>(window_context_.width) / 2.0f) / static_cast<float>(window_context_.width);
  logic_context_.event.cursor_event.ypos =
          (static_cast<float>(window_context_.hight) / 2.0f - ypos) / -static_cast<float>(window_context_.hight);
}
...
void LogicEngine::CharInputCallback(char key, int action) {
  if (action == GLFW_PRESS) {
    ...
    std::erase_if(logic_context_.event.key_event, [key](char c){return key == c;});
  }
}

LogicContext &LogicEngine::GetLogicContext() {
  return logic_context_;
}
