...
PodBullet::PodBullet()
    : base::GameObject("pod_bullet"),
      start_model_(glm::mat4(1.0f)),
      translation_matrix_(glm::mat4(1.0f)),
      speed_(0.0f) {}

PodBullet::PodBullet(std::shared_ptr<Object> other)
    : base::GameObject(std::dynamic_pointer_cast<base::GameObject>(other)),
      start_model_(glm::mat4(1.0f)),
      translation_matrix_(glm::mat4(1.0f)) {
  std::shared_ptr<PodBullet> dother =
      std::dynamic_pointer_cast<PodBullet>(other);
...
render::ObjectRenderData PodBullet::GetRenderData() {
  ...
  render_data.vertices = {
      0.018f, 0.0f,  0.001f, -0.018f, 0.0f,  0.001f,
      0.018f, 0.12f, 0.001f, -0.018f, 0.12f, 0.001f,
  };
  render_data.indices = {0, 1, 2, 1, 2, 3};
  gl_element_cout_ = render_data.indices.size();

  physics::RectangleBox rectangle_box;
  for (int i = 0; i < 4; ++i) {
    rectangle_box.vertices[i] = glm::vec4(
        render_data.vertices[i * 3 + 0], render_data.vertices[i * 3 + 1],
        render_data.vertices[i * 3 + 2], 1.0f);
  }
  SetBoundingBox(physics::BBoxType::rectangle,
                 glm::vec4(0.0f, 0.06f, 0.0f, 1.0f), rectangle_box);

  return render_data;
}

void PodBullet::PrebuildShader() {
  shader_.PrebuildFile("../src/atn/game/shader/pod_bullet.vs",
