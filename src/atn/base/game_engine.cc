...
void GameEngine::Initialize(WindowContext* window_context) {
  ...
  game_control_ = std::make_shared<game::GameControl>(logic_engine_->GetLogicContext(), render_engine_->GetRenderContext());
...
void GameEngine::Tick() {
  logic_engine_->GetLogicContext().interval_time =
...
void GameEngine::RemoveGlobalObjectCache(std::shared_ptr<Object> object_cache) {
  std::string tag = object_cache->Tag();
  std::erase_if(global_objects_cache_map_, [&tag](const auto& item) {
    auto const& [key, value] = item;
    return key.compare(tag) == 0;
  });
}
...
void GameEngine::SetFps(int fps) { fps_ = fps; }
...
void GameEngine::Sleep() {
  ...
  std::chrono::milliseconds sleep_time =
      per_frame_time - current_tick_duration_;
